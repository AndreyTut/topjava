...
public class UserMealsUtil {
    ...
    public static List<UserMealWithExcess> filteredByCycles(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        ...
        for (UserMeal meal : meals) {
            calsPerDay.merge(meal.getDateTime().toLocalDate(), meal.getCalories(), (ov, nv) -> ov + nv);
            if (TimeUtil.isBetweenInclusive(meal.getDateTime().toLocalTime(), startTime, endTime)) {
                userMeals.add(meal);
            }
        }
        return userMeals.stream()
                .map(userMeal -> createMealWithExcess(userMeal, calsPerDay.get(userMeal.getDateTime().toLocalDate()) > caloriesPerDay))
                .collect(Collectors.toList());
    }

    public static List<UserMealWithExcess> filteredByStreams(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        Map<LocalDate, Integer> mapDaysCalors = meals.stream()
        ...
        return meals.stream()
                ...
                .map(meal -> createMealWithExcess(meal, mapDaysCalors.get(meal.getDateTime().toLocalDate()) > caloriesPerDay))
                .collect(Collectors.toList());
    }
